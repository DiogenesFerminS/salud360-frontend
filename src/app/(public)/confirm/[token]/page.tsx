...
const Page = ({ params } : {params: Promise<{token: string}>} ) => {
  ...
  useEffect(() => {
    const comfirmAccount = async()=>{
      ...
      try {
        const url = `${process.env.NEXT_PUBLIC_URL_BACKEND}/api/users/confirm/${token}`
  ...
  return (
    <div className=" min-h-screen flex flex-col justify-center items-center md:flex-row gap-8 p-3">
        <section className="max-w-96 flex animate-fade-in-right animate-delay-600 p-1 lg:max-w-150">
             ...
             <div>
                  {
                    ...
                    : (
                      <h1 className="text-4xl font-bold text-gray-50 text-center">Confirm your account to start <span className="text-cyan-400">managing</span> your clients</h1>
                    )
                  }
        ...
        <section className="bg-gray-50 p-4 rounded-lg shadow-lg animate-fade-in-up max-w-90">
              {
                (hasError.isActive)
